...
export class ZohoAuthenticationProvider {
  ...
  public async login(): Promise<vscode.AuthenticationSession | undefined> {
    try {
      ...
      const authSession = await this.startLocalServer(
        state,
        clientId,
        clientSecret,
        apiDomain
  ...
  private startLocalServer(
    ...
    return new Promise((resolve, reject) => {
      ...
      ZohoAuthenticationProvider.server = http.createServer(
        async (req, res) => {
          try {
            if (!req.url) {
      ...
      // Iniciar el servidor
      ZohoAuthenticationProvider.server.listen(
        ZohoAuthenticationProvider.serverPort,
        () => {
          console.log(
            `Servidor de autenticación iniciado en puerto ${ZohoAuthenticationProvider.serverPort}`
          );
        }
      );
    });
